...
def start(message):
	...
	bot.send_message(message.chat.id, 'Привет! Меня зовут Roomba. Я подбираю идеальных тебе соседей для совместной аренды жилья и помогаю людям, которые уже имеют жилье или его снимают, найти себе идеальных клиентов.\n\n Для начала нашей работы выбери что ты хочешь сделать: Создать объявление, Просмотреть объявления, Просмотреть свое объявление или изменить его, Оставить свою обратную связь по моей работе и как ее улучшить', reply_markup=keyboard)
	...
	if call.message:
		if call.data == 'profile_next' or call.data == 'profile_prev' or call.data == 'rematch_profiles':
			...
			if u.profiles is None or u.cur_profile >= len(u.profiles) or u.cur_profile < 0:
				bot.send_message(call.message.chat.id, 'Ошибка. Попробуй еще раз.')
				return
...
def name_insert_data(message):
	...
	if message.text == 'Добавить новое объявление':
		...
		if u.mode == 1:
			...
			keyboard = types.ReplyKeyboardMarkup(True, True)
			keyboard.row('🔙Назад в меню')
			bot.send_message(message.chat.id, 'Как бы грубо не звучало, но скажи мне свой возраст :)', reply_markup = keyboard)
		elif u.mode == 2:
			age = message.text
			if not age.isdigit() or int(age) > 110 or int(age) < 14:
				bot.send_message(message.chat.id, 'Неправильный ввод! Введи целое число.')
				return
			...
			if message.text == 'Мужчина':
				...
				keyboard.row('Мужчина', 'Женщина')
				keyboard.row('🔙Назад в меню')
				bot.send_message(message.chat.id, 'Неправильный ввод! Прошу тебя воспользоваться клавиатурой', reply_markup = keyboard)
				return
			u.mode += 1
			keyboard = types.ReplyKeyboardMarkup(True, True)
			...
			if message.text == 'учусь':
				...
				keyboard.row('Евразийский НУ', 'Международный Университет Астаны')
			...
			if lang == 'Казахский' or lang == 'Русский' or lang == 'Оба языка': 
				...
				keyboard.row('🔙Назад в меню')
				bot.send_message(message.chat.id, 'Укажи свои вредные привычки .Я если что люблю людей ведущих ЗОЖ :)', reply_markup=keyboard)
			else:
				bot.send_message(message.chat.id, 'Неправильный ввод!')
				return
		elif u.mode == 9:
			...
			if distr == 'Алматинский' or distr == 'Байконурский' or distr == 'Есильский' or distr == 'Сарыаркинский':
				...
				if u.seeker.hata == True:
					...
					bot.send_message(message.chat.id, 'Уточни возле чего тебе удобно жить(название \
			...
			keyboard = types.ReplyKeyboardMarkup(True, True)
			keyboard.row('🔙Назад в меню')
			bot.send_message(message.chat.id, 'Расскажи о себе. Можешь сказать свои интересы, хобби, что любишь читать, какие фильмы смотреть. Хочу узнать о тебе побольше.',reply_markup=keyboard)
		elif u.mode == 15:
			...
			keyboard = types.ReplyKeyboardMarkup(True, True)
			keyboard.row('🔙Назад в меню')
			bot.send_message(message.chat.id, 'Отправь своё селфи. Но только не стесняйся. Я всех люблю', reply_markup=keyboard)
			u.mode += 1
		elif u.mode == 17:
